...
- **Any task:** move through Part 3 (Execution Loop) in order — orient, pin intent, locate,
  plan the change, change surgically, debug systematically, verify by running.
...
- **Feeling a shortcut coming on:** check Part 4 (Failure Modes) — if your next move matches
  a trap row, apply its hard rule instead.
...
No edit to unread code. No claim of "done" without command output. No diff bigger than the
task. No API you haven't verified exists. Assumptions stated out loud, every time.
